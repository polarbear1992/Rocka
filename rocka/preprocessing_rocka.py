# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 15:07:20 2019
...
__all__ = ['linear_interpolation','standardize_obj']

def linear_interpolation(timestamp, arrays=None):
    """
    ...
    Linear interpolation would be utilized for missing points(0) and 
    default points(-1)
    Also, an indicator array will be returned to indicate whether each
    point is missing or defauld.
    ...
    #linear interpolation for the missing values and the default values(-1)
    neg_indexs = [np.argwhere(array==-1) for array in ret_arrays]
    miss_index = np.argwhere(ret_missing==1)
    
    for neg_index,ret_array in zip(neg_indexs,ret_arrays):
        if len(neg_index) > 0:
